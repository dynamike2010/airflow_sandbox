...
dag = DAG(
    ...
    description='A DAG that waits for ETL DAG to complete successfully',
...
wait_for_etl_success = RecentExternalTaskSensor(
    task_id='wait_for_etl_success',
    external_dag_id='dbt_build_monolith',
    external_task_id='end_task',
    trigger_when=Trigger.OnSuccess,
...
def print_message():
    print("Running after SUCCESS of ETL DAG!")
    print(f"Current time: {datetime.now()}")
    return "Success message printed"
...
restart_self = TriggerDagRunOperator(
    ...
    trigger_dag_id='etl_success_sensor',
    reset_dag_run=True,
    wait_for_completion=False,
    dag=dag
)

wait_for_etl_success >> perform_task >> delay >> restart_self
